...
class FileSearchTool:
    ...
    def _excluded(self, path: Path) -> bool:
        ...
        return any(fnmatch.fnmatch(text, pattern) or any(fnmatch.fnmatch(part, pattern.rstrip("/**")) for part in path.parts) for pattern in self.excludes)
    ...
    def search_text(self, terms: list[str], limit: int = 12) -> list[str]:
        ...
        found: list[str] = []
        for path in self.root.rglob("*"):
            if not path.is_file() or self._excluded(path) or path.stat().st_size > 1_000_000:
                continue
            try:
