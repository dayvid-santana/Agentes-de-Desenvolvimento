...
| `headers` | `enabled`, `author`, `date_format`, `history` | `HeaderService` só acrescenta cabeçalho a arquivos suportados sem cabeçalho. `history` existe no modelo, mas o serviço atual não usa esse campo. |
