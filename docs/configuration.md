...
| `headers` | `enabled`, `author`, `date_format`, `history` | `HeaderService` só acrescenta cabeçalho a arquivos suportados sem cabeçalho. `dev-agent headers --check` lista os candidatos do escopo de contexto, e `--apply --confirm` os altera. `history` existe no modelo, mas o serviço atual não usa esse campo. |
